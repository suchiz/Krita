...
KisToolMultihand::KisToolMultihand(KoCanvasBase *canvas)
    : KisToolBrush(canvas),
      ...
      m_translateRandomly(false),
      m_translateHorizontally(true),
      m_translateVertically(false),
      m_showAxes(false),
      m_translateRadius(200),
...
void KisToolMultihand::initTransformations()
{
    ...
    if(m_transformMode == SYMMETRY) {
        ...
        }else if(m_translateVertically) {
	    qreal length = m_translateRadius/m_handsCount;
	    qreal shift = -length*(m_handsCount/2);
...
QWidget* KisToolMultihand::createOptionWidget()
{
    ...
    customUI->translationRadiusSpinbox->setRange(0, 800);
    ...
    customUI->translateRandomlyCheckbox->setChecked((bool)m_configGroup.readEntry("translateRandomly", false));
    customUI->translateHorizontalCheckbox->setChecked((bool)m_configGroup.readEntry("translateHorizontally", false));
    customUI->translateVerticalCheckbox->setChecked((bool)m_configGroup.readEntry("translateVertically", false));
    
    connect(customUI->translateRandomlyCheckbox, SIGNAL(toggled(bool)), this, SLOT(slotSetTranslateRandomly(bool)));
    connect(customUI->translateVerticalCheckbox, SIGNAL(toggled(bool)), this, SLOT(slotSetTranslateVertically(bool)));
    connect(customUI->translateHorizontalCheckbox, SIGNAL(toggled(bool)), this, SLOT(slotSetTranslateHorizontally(bool)));
...
void KisToolMultihand::slotSetTransformMode(int index)
{
    ...
    customUI->translateRandomlyCheckbox->setVisible(false);
    customUI->translateHorizontalCheckbox->setVisible(false);
    customUI->translateVerticalCheckbox->setVisible(false);
    ...
    if (index == int(MIRROR)) {
     ...
     if (index == int(TRANSLATE)) {
       customUI->translateRandomlyCheckbox->setVisible(true);
       customUI->translateHorizontalCheckbox->setVisible(true);
        customUI->translateVerticalCheckbox->setVisible(true);
        customUI->translateHorizontalCheckbox->setChecked(true);
        customUI->translateVerticalCheckbox->setChecked(false);
       customUI->translateRandomlyCheckbox->setChecked(false);
...
void KisToolMultihand::slotSetTranslateRandomly(bool random)
{
    m_translateRandomly = random;
    m_configGroup.writeEntry("translateRandomly", random);
    
    if (random){
       customUI->translateHorizontalCheckbox->setChecked(false);
       customUI->translateVerticalCheckbox->setChecked(false);
    }
}


void KisToolMultihand::slotSetTranslateVertically(bool random)
{
    m_translateVertically= random;
    m_configGroup.writeEntry("translateVertically", random);
...
void KisToolMultihand::slotSetTranslateHorizontally(bool random)
{
    m_translateHorizontally = random;
    m_configGroup.writeEntry("translateHorizontally", random);
